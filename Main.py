        ...
        if validation_of_data(self.tableWidget):
            ...
            while i < self.tableWidget.rowCount():
                ...
                try:
                    ...
                    tg_sum_k_i = math.tan(sum_of_k_i)
                    factorial *= i
                    answer = ((int(item) ** 2 + int(item_minus_1)**5)**(1/3.0))/(tg_sum_k_i**(1/2.0))
                    answer1 = answer+factorial

                    self.tableWidget.setItem(i, j,
                                             QTableWidgetItem(str(format(answer1, ".10f"))))
                except Exception:
